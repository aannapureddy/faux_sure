...
from typing import Any, Callable, Dict, List, Iterable, Optional, T, Tuple, Union, Set, Generator

from faux_orm.exceptions import (
    ModelNotFoundError, 
...
class Field:
    ...
    def __init__(
        ...
        self._type: T = _type
        self.validators: Tuple[ValidatorType] = tuple(validators) if isinstance(validators, Iterable) else (validators,)
    ...
    def __set__(self, instance, value):
        ...
        if not isinstance(value, self._type):
            raise TypeFieldRequirementException(f"{self.name!r} values must one of types {self._type!r} not {type(value)}"})
...
class UniqueTogetherRestraint:
    ...
    def __init__(self, unique_fields: Union[str, Iterable[str]]):
        ...
        self.unique_fields: Tuple[str] = tuple(unique_fields) if isinstance(unique_fields, Iterable) or (unique_fields,)
...
class Model:
    ...
    def iterate_model_fields(self) -> Generator[Field, None, None]:
        """
    ...
    def iterate_unique_constraints(self) -> Generator[Tuple[str], None, None]:
        """
        Iterate over all unique constraints and yield values together
        
        Raises Attribute Exception if field does not exist on Model
        """
        unique_constraints = (c_field for c_field in self.__dict__.values() if isinstance(c_field, UniqueTogetherRestraint))
...
class Session:
    ...
    def query_by_key(cls, pk: str) -> Model:
        ...
        try:
            return cls.Indexes.model_index[key]
        except KeyError:
    ...
    def remove(cls, pk: str) -> bool:
        ...
        try:
            ...
            return False
        del cls.Indexes.model_index[key]
        cls.Indexes.class_index[model.class_name].remove(key)
        return True
